      ...
      success: (response) => {
        ...
        for (let i = 0; i < 50; i++) {
          trackInfo[type].push({
            ...
            url: response["items"][i]["album"]["images"][2]["url"],
      ...
      success: (response) => {
        for (let i = 0; i < 50; i++) {
          artistInfo[type].push({
            uri: response["items"][i]["uri"],
            url: response["items"][i]["images"][2]["url"],
            name: response["items"][i]["name"],
          });
        }
      ...
      success: (response) => {
        ...
        for (let i = 0; i < 50; i++) {
          recentInfo.push({
            ...
            url: response["items"][i]["track"]["album"]["images"][2]["url"],
          ...
          <div>
            ...
            <Container fluid>
              ...
              {tracks === "none" && artists === "none" ? (
                ...
                <Titles
                  condition={tracks}
                  title={displayName + "'s Top 50 Tracks"}
                />
              ) : (
                <Titles
                  condition={artists}
                  title={displayName + "'s Top 50 Artists"}
                />
              )}
              ...
              {tracks !== "none" && (
                <button
                  className="btn btn-playlist btn-md"
                  onClick={() =>
                    this.createPlaylist(
                      token,
                      id,
                      "Top 20 Tracks (" + ranges[tracks] + ")",
                      String(new Date()),
                    )
                  }
                >
